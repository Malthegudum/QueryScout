...
class DSTClient:
    ...
    def tables(self, subject: str | None = None):
        ...
        response = requests.get(f"{self.BASE_URL}/tables", params=params)
        response.raise_for_status()
        return response.json()

    def table(self, table_id: str):
        response = requests.get(f"{self.BASE_URL}/tableinfo", params={"id": table_id})
        response.raise_for_status()
        return response.json()

    def build_request(self, table_id: str, variables: dict[str, list[str]]) -> RequestSpec:
        request_variables = [
        ...
        return RequestSpec(
            ...
            json={"table": table_id, "format": "CSV", "variables": request_variables},
        )
